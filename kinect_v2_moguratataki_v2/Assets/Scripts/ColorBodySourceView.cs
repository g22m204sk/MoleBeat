...
public class ColorBodySourceView : MonoBehaviour
{
    ...
    private Dictionary<ulong, GameObject> _Bodies = new Dictionary<ulong, GameObject>();//キーと一緒にかくJoint情報を格納
    private BodySourceManager _BodyManager;

    public Camera ConvertCamera;
    private Kinect.CoordinateMapper _coordinatemapper;
    private int KinectHeight = 1920;
    private int KinectWidth = 1080;
    ...
    void Update()
    {
        ...
        Kinect.Body[] data = _BodyManager.GetData();//センサで認識した数だけ配列の要素を作る
        ...
        // First delete untracked bodies　トラッキングIDが含まれなかったら消す
        ...
        /*
        var bodya = _Bodies.Values;
        foreach (var i in bodya)
        {
            for (int j = 0; j < i.transform.childCount; j++)
            {
                Debug.Log(i.transform.GetChild(j).name);
    ...
    private Vector3 GetVector3FromJoint(Kinect.Joint joint)
    {
        ...
        if (ConvertCamera != null || valid)
        {
            ...
            if ((0 <= point2.x) && (point2.x < KinectWidth) &&
                 (0 <= point2.y) && (point2.y < KinectHeight))
            {

                // スクリーンサイズで調整(Kinect->Unity)
                point2.x = point2.x * Screen.width / KinectWidth;
                point2.y = point2.y * Screen.height / KinectHeight;
